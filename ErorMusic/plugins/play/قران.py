...
async def quran(c,msg):
    quran = json.loads(open("ErorMusic/assets/quran.json").read())["s"] # ملف القران الي بستخدمه بتاع @Y_D_ll 
    keyboard = []
    list = []
    for i in range(1,11):
        if len(list) == 2:
