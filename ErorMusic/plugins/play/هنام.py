...
@app.on_message(filters.command(["تصبحي ع خير","تصبح ع خير",], prefixes=["","E"]))
def goodnight_command_handler(_, message):
    ...
    if send_sticker:
        ...
        message.reply_text(f"**ليله سعيده, {sender}! نام كويس 🌙**")
    else:
        emoji = get_random_emoji()
        app.send_message(message.chat.id, emoji)
        message.reply_text(f"**ليله سعيده, {sender}! نام كويس 🌙. {emoji}**")
